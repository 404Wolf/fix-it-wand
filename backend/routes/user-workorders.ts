...
export const userWorkordersRoute = new Hono();

// Protect all routes with JWT authentication
userWorkordersRoute.use("*", jwt({ secret: JWT_SECRET, cookie: "auth_token" }));

// Get all workorders for the authenticated user
userWorkordersRoute.get("/", async (c) => {
...
userWorkordersRoute.post("/", async (c) => {
  const payload = c.get("jwtPayload");
  const userEmail = payload.email;
  const user = await getUserByEmail(userEmail);
...
userWorkordersRoute.post("/:id/send", async (c) => {
  const payload = c.get("jwtPayload");
  const userEmail = payload.email;
  const user = await getUserByEmail(userEmail);
...
userWorkordersRoute.post("/:id/complete", async (c) => {
  const payload = c.get("jwtPayload");
  const userEmail = payload.email;
  const user = await getUserByEmail(userEmail);
  const workorderId = c.req.param("id");
...
// Delete a workorder
userWorkordersRoute.delete("/:id", async (c) => {
  const payload = c.get("jwtPayload");
  const userEmail = payload.email;
  const user = await getUserByEmail(userEmail);
  const workorderId = c.req.param("id");
